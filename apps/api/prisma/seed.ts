...
async function main() {
  ...
  await prisma.passwordReset.deleteMany();   // ← NEW
  ...
  const sarah = await prisma.user.create({ data: { email: 'robamanwa@gmail.com', passwordHash: staffPassword, name: 'Sarah Johnson', role: Role.STAFF, companyId: company.id, canAccessAllBranches: false, mustChangePassword: true } });
  const james = await prisma.user.create({ data: { email: 'james@arizonamedtransport.com', passwordHash: staffPassword, name: 'James Williams', role: Role.STAFF, companyId: company.id, canAccessAllBranches: false, mustChangePassword: true } });
  const maria = await prisma.user.create({ data: { email: 'maria@arizonamedtransport.com', passwordHash: staffPassword, name: 'Maria Garcia', role: Role.STAFF, companyId: company.id, canAccessAllBranches: false, mustChangePassword: true } });
  ...
  function makeTime(base: Date, hour: number, minute = 0): Date { const d = new Date(base); d.setHours(hour, minute, 0, 0); return d; }

  await prisma.shift.createMany({
    data: [
      { patientId: helen.id, branchId: phoenixBranch.id, hospitalId: stJosephs.id, startTime: makeTime(tomorrow, 9, 0), endTime: makeTime(tomorrow, 10, 30), type: MeetingType.PHYSICAL, priority: Priority.NORMAL, notes: 'Routine cardiology checkup' },
      { patientId: william.id, branchId: phoenixBranch.id, hospitalId: stJosephs.id, startTime: makeTime(tomorrow, 11, 0), endTime: makeTime(tomorrow, 12, 0), type: MeetingType.PHYSICAL, priority: Priority.URGENT, notes: 'Post-surgery follow-up — needs wheelchair transport' },
      { patientId: dorothy.id, branchId: tucsonBranch.id, hospitalId: bannerUMC.id, startTime: makeTime(tomorrow, 14, 0), endTime: makeTime(tomorrow, 15, 30), type: MeetingType.PHYSICAL, priority: Priority.NORMAL, notes: 'Dialysis session' },
      { patientId: richard.id, branchId: mesaBranch.id, hospitalId: mercyGilbert.id, startTime: makeTime(nextWeek, 10, 0), endTime: makeTime(nextWeek, 11, 0), type: MeetingType.PHYSICAL, priority: Priority.NORMAL, notes: 'Orthopedic consultation' },
      { patientId: margaret.id, branchId: phoenixBranch.id, startTime: makeTime(nextWeek, 13, 0), endTime: makeTime(nextWeek, 13, 30), type: MeetingType.VIRTUAL, priority: Priority.NORMAL, notes: 'Telehealth follow-up with primary care' },
    ],
  });
